...
import { createContext, FormEvent, ReactNode, useContext, useMemo, useState, useTransition } from "react";
...
const TaskCommentCountsContext = createContext<Map<number, number>>(new Map());

function TaskCommentCountsProvider({ children }: { children: ReactNode }) {
    const { threads } = useThreads({ query: { metadata: { kind: "kanban-task" } }, scrollOnLoad: false });
    ...
    const counts = useMemo(() => {
        ...
        for (const thread of threads ?? []) {
            ...
            const live = thread.comments.filter((comment) => !comment.deletedAt).length;
            map.set(taskId, (map.get(taskId) ?? 0) + live);
        }
        return map;
    }, [threads]);

    return <TaskCommentCountsContext.Provider value={counts}>{children}</TaskCommentCountsContext.Provider>;
}
...
function TaskCard({
    ...
    const commentCount = useContext(TaskCommentCountsContext).get(task.id) ?? 0;
